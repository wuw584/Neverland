...
# 创建文件夹
if not os.path.exists(event_cut_plt_save_dir):
    os.makedirs(event_cut_plt_save_dir)	
if not os.path.exists(event_cut_npy_save_dir):
    os.makedirs(event_cut_npy_save_dir)

##catalog file
cat = pd.read_csv(event_catalog_file, delim_whitespace=True)
# print(cat)
...
plt.savefig(event_cut_plt_save_dir + "event_map.png")
...
save_file_name_prefix =  event_cut_npy_save_dir + event_cut_file_name_prefix
ieq_list = dce.extract_das_data(flist, cat_time, dt_before, dt_after, save_file_name_prefix, overwrite=True, verbose=True)
...
for ieq in ieq_list:
    ...
    # plt.xlabel('Time (s)')
    save_filename = save_file_name_prefix+str(ieq)
    # if not os.path.exists(save_filename):
    #     os.system(r"touch {}".format(save_filename)) #调用系统命令行来创建文件

    plt.savefig( os.path.join(event_cut_plt_save_dir, event_cut_file_name_prefix +str(ieq) + ".png" ))
    plt.close()